...
import json
from dbconf import PenDBConfigModel

class PenAdmConfigModel(BaseModel):
    log_file: str
    ...
    allow_ip_addrs: List[str] = [
            "127.0.0.1",
            "::1",
            ]
    lab_copy_interval: int = 600   # 10 minutes
    logger: Any
    loop: Any
...
def set_config(prog_name, loop, args=None):
    ...
    try:
        config = PenAdmConfigModel.parse_obj(json.load(open(config_file)))
...
if __name__ == "__main__":
    ...
    m = PenAdmConfigModel.parse_obj(conf)
    print(m)
